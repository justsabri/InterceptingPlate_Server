...
std::vector<uint8_t> MotorParser::receive(uint8_t cmd) {
    ...
    if(frame.data[0] == 0x0a){
        ...
        if (status & (1 << 0)) AERROR << " - Internal software error\n";
        if (status & (1 << 1)) AERROR << " - Overvoltage\n";
        if (status & (1 << 2)) AERROR << " - Undervoltage\n";
        if (status & (1 << 4)) AERROR << " - Startup error\n";
        if (status & (1 << 5)) AERROR << " - Speed feedback error\n";
        if (status & (1 << 6)) AERROR << " - Overcurrent\n";
        if (status & (1 << 7)) AERROR << " - Other software error\n";
        if (status & (1 << 16)) AERROR << " - Encoder communication error\n";
        if (status & (1 << 17)) AERROR << " - Motor over temperature\n";
        if (status & (1 << 18)) AERROR << " - Board over temperature\n";
        if (status & (1 << 19)) AERROR << " - Driver chip error\n";
...
std::string MotorParser::getMotorMode(int can_id) {
    ...
    switch (mode) {
        case -1: return "Fault mode";
        case 0: return "Stop mode";
        case 1: return "Current mode";
        case 2: return "Speed mode";
