...
float IMURS232::parseFloat(const uint8_t *data) {
    union {
        uint8_t bytes[4];
        float value;
    } u;
...
double IMURS232::parseDouble(const uint8_t *data) {
    union {
        uint8_t bytes[8];
        double value;
    } u;
...
void IMURS232::parserData(uint8_t *data){  // 根据协议图解析数据
    // 跳过帧头和数据长度，直接解析有效数据
    imu_data.left_rudder = parseFloat(data + 4);    // 左桨舵角
    imu_data.right_rudder = parseFloat(data + 8);  // 右桨舵角
    imu_data.speed = parseFloat(data + 12);        // 航速
    imu_data.timestamp = parseInt32(data + 16);    // UNIX时间戳
    imu_data.longitude = parseDouble(data + 20);   // 经度
    imu_data.latitude = parseDouble(data + 28);    // 纬度
    imu_data.left_rpm = parseFloat(data + 36);     // 左机转速
    imu_data.right_rpm = parseFloat(data + 40);    // 右机转速
    imu_data.left_gear = parseUint16(data + 44);   // 左机挡位
    imu_data.right_gear = parseUint16(data + 46);  // 右机挡位
    // CRC校验（可选，用于验证数据完整性）
    uint32_t crc32 = parseUint32(data + 48);
}
